...
python << PYTHONEOF
import sys, tokenize, cStringIO, types, locale
...
def vimcomplete(context,match):
    ...
    try:
        ...
        dictstr += ']'
        #dbg("dict: %s" % dictstr)
        vim.command("silent let g:pythoncomplete_completions = %s" % dictstr.decode(locale.getdefaultlocale()[1], "replace").encode(vim.eval("&enc"), "replace"))
        #dbg("Completion dict:\n%s" % all)
    except vim.error:
        dbg("VIM Error: %s" % vim.error)
        vim.command("silent let g:pythoncomplete_completions = []")
...
class Completer(object):
    ...
    def get_arguments(self,func_obj):
        ...
        elif type(func_obj) == types.TypeType and hasattr(func_obj, "__init__"): func_obj = func_obj.__init__.im_func
        else: arg_offset = 0
        ...
        if type(func_obj) in [types.FunctionType, types.LambdaType]:
            try:
                ...
                defaults = map(lambda name: "=%s" % repr(name), defaults)
    ...
    def get_completions(self,context,match):
        ...
        for k, v in self.compldict["__builtins__"].items():
            if k not in self.compldict:
                self.compldict[k] = v
        try:
            result = None
            all = {}
            ...
            if len(stmt) > 0 and stmt[-1] == '(':
                ...
                try:
                    ...
                    doc = ""
                if doc is None: doc = ''
                args = self._cleanstr(self.get_arguments(result))
                return [{'word': " " if args != ")" else ")", 'abbr': result.__name__ + ('(...)' if args != ")" else '()'), 'kind':'f', 'info':'function ' + result.__name__ + "(" + args + "\n" + self._cleanstr(doc)}]
            elif ridx == -1:
                match = stmt
                all = self.compldict
            else:
            ...
            for m in all:
                ...
                try:
                    ...
                    if m.find(match) == 0:
                        ...
                        if doc is None or doc == '': doc = ""

                        t = type(inst)
                        wrd = m[len(match):]
                        c = {'word':wrd}
                        if t in [types.FunctionType, types.LambdaType, types.MethodType, types.BuiltinFunctionType, types.BuiltinMethodType]:
                            args = self._cleanstr(self.get_arguments(inst))
                            c['word'] += '(' if args != ")" else "()"
                            c['abbr'] = m + ('(...)' if args != ")" else "()")
                            c['info'] = 'function {}({}\n{}'.format(m, args, self._cleanstr(doc))
                            c['kind'] = 'f'
                        elif t == types.ModuleType:
                            c['abbr'] = m
                            c['info'] = 'module {}\n{}'.format(m, self._cleanstr(doc))
                            c['kind'] = 'm'
                            ...
                            #get constructor
                            cc = {}
                            if hasattr(inst, "__init__") and "wrapper" not in str(inst.__init__):
                                try:
                                    doc = inst.__init__.__doc__
                                except:
                                    doc = ""
                                if doc is None:
                                    doc = ""
                                args = self._cleanstr(self.get_arguments(inst))
                                cc['word'] = wrd + ('(' if args != ")" else "()")
                                cc['abbr'] = m + ('(...)' if args != ")" else "()")
                                cc['kind'] = 'f'
                                cc['info'] = "constructor {}.__init__({}\n{}".format(m, args, self._cleanstr(doc))
                            else:
                                cc['word'] = wrd + '('
                                cc['abbr'] = m + '(...)'
                                cc['kind'] = 'f'
                            ...
                            c['abbr'] = m
                            c['info'] = "variable {}\ntype {}".format(m, type(inst).__name__)
                            c['kind'] = 'v'
                        completions.append(c)
                except:
                    i = sys.exc_info()
